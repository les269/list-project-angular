...
export enum ValuePipelineType {
  ...
  COPY_SPECIFIED_VALUE_TO = 'COPY_SPECIFIED_VALUE_TO', // 將指定的鍵值複製到指定的鍵值點
  DELETE = 'DELETE', // 刪除字串或是移除陣列元素
  DELETE_PATHS = 'DELETE_PATHS', // 刪除JSON中的特定路徑
  MOVE_CHAR = 'MOVE_CHAR', // 移動字元位置
  JOIN_ARRAY = 'JOIN_ARRAY', // 將陣列元素合併成字串
...
export interface ValuePipeline {
  ...
  chineseConvertType: ChineseConvertType; // 當 type 是 CHINESE_CONVERT 時使用，指定轉換類型
  insertConfig: InsertConfig; // 當 type 是 INSERT 時使用，指定要插入的字串或陣列元素
  copySpecifiedValueToConfig: CopySpecifiedValueToConfig; // 當 type 是 COPY_SPECIFIED_VALUE_TO 時使用，指定要複製的鍵值對
...
export interface InsertConfig {
  ...
  key: string; // 當插入位置是根據鍵值時，指定要插入的鍵值點
  index: number; // 當 position 是 NTH 時，指定插入位置的索引（從0開始）
  text: string; // 要插入的字串
}

export interface CopySpecifiedValueToConfig {
  copyKey: string; // 要複製的鍵值
  insertKey: string; // 要插入的鍵值點
}

export interface DeleteConfig {
  position: PositionType; // 刪除位置，開始或結束
  key: string; // 當刪除位置是根據鍵值時，指定要刪除的鍵值點
